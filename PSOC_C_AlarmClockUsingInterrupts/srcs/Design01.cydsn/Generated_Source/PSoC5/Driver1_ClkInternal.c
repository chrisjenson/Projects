/*******************************************************************************
* File Name: Driver1_ClkInternal.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_Start
********************************************************************************
*
* Summary:
...
void Driver1_ClkInternal_Start(void) 
{
    ...
    Driver1_ClkInternal_CLKEN |= Driver1_ClkInternal_CLKEN_MASK;
	Driver1_ClkInternal_CLKSTBY |= Driver1_ClkInternal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_Stop
********************************************************************************
*
* Summary:
...
void Driver1_ClkInternal_Stop(void) 
{
    ...
    Driver1_ClkInternal_CLKEN &= (uint8)(~Driver1_ClkInternal_CLKEN_MASK);
	Driver1_ClkInternal_CLKSTBY &= (uint8)(~Driver1_ClkInternal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Driver1_ClkInternal_CLKEN & Driver1_ClkInternal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Driver1_ClkInternal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Driver1_ClkInternal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Driver1_ClkInternal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Driver1_ClkInternal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Driver1_ClkInternal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Driver1_ClkInternal_GetSourceRegister();
    uint16 oldDivider = Driver1_ClkInternal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Driver1_ClkInternal_CLKEN & Driver1_ClkInternal_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(Driver1_ClkInternal_DIV_PTR, clkDivider);
                Driver1_ClkInternal_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                Driver1_ClkInternal_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(Driver1_ClkInternal_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Driver1_ClkInternal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(Driver1_ClkInternal_DIV_PTR, clkDivider);
				Driver1_ClkInternal_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Driver1_ClkInternal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Driver1_ClkInternal_MOD_SRC |= modeBitMask & (uint8)Driver1_ClkInternal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Driver1_ClkInternal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Driver1_ClkInternal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Driver1_ClkInternal_MOD_SRC & (uint8)(Driver1_ClkInternal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Driver1_ClkInternal_GetDividerRegister();
    uint8 oldSrc = Driver1_ClkInternal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Driver1_ClkInternal_MOD_SRC |= CYCLK_SSS;
        Driver1_ClkInternal_MOD_SRC =
            (Driver1_ClkInternal_MOD_SRC & (uint8)(~Driver1_ClkInternal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Driver1_ClkInternal_MOD_SRC & Driver1_ClkInternal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Driver1_ClkInternal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Driver1_ClkInternal_PHASE = clkPhase & Driver1_ClkInternal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Driver1_ClkInternal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Driver1_ClkInternal_PHASE & Driver1_ClkInternal_PHASE_MASK;
}
