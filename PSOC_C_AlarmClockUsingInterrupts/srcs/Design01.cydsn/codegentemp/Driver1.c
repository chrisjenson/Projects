...
#include <Driver1_DMA_Seg_dma.h>
    ...
    #include <Driver1_DMA_BC_dma.h>
#endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */

/* Function prototypes */
static void Driver1_DmaInit(void) ;
static void Driver1_DmaEnable(void) ;
static void Driver1_DmaDisable(void) ;

/* Used to hold the common line output sequence */
#if(Driver1_COM_DRIVE == Driver1_HIGH)
    static uint8 Driver1_comBuffer[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u, 0x00u, 0x08u,
                                                    0x00u, 0x10u, 0x00u, 0x20u, 0x00u, 0x40u, 0x00u, 0x80u};
#else
    static uint8 Driver1_comBuffer[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu, 0xFFu, 0xF7u,
                                                    0xFFu, 0xEFu, 0xFFu, 0xDFu, 0xFFu, 0xBFu, 0xFFu, 0x7Fu};
#endif /* (Driver1_COM_DRIVE == Driver1_HIGH) */

/* Buffers used to hold the display(segment) values to be transferred to the control registers */
uint8 Driver1_segLBuffer[Driver1_NUM_COM];
#if (Driver1_NUM_SEG > 8u) 
    uint8 Driver1_segMBuffer[Driver1_NUM_COM];
#endif /* (Driver1_NUM_SEG > 8u) */
#if (Driver1_NUM_SEG > 16u) 
    uint8 Driver1_segHBuffer[Driver1_NUM_COM];
#endif /* (Driver1_NUM_SEG > 16u) */

#if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
    /* The brightness register used to hold the brightness levels for each common */
    static uint8 Driver1_brightnessReg[Driver1_NUM_COM];
#endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */

/* DMA channel and TD declaractions */
static uint8 Driver1_dmaComChannel;
static uint8 Driver1_dmaComTd[Driver1_TWO_NUM_COM];

static uint8 Driver1_dmaSegChannel;
static uint8 Driver1_dmaSegLTd[Driver1_NUM_COM];

#if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
    static uint8 Driver1_dmaBCChannel;
    static uint8 Driver1_dmaBCTd;
#endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */

uint8 Driver1_initVar = 0u;
uint8 Driver1_enableVar = 0u;


/*******************************************************************************
* Function Name: Driver1_DmaInit
********************************************************************************
*
* Summary: 
...
static void Driver1_DmaInit(void) 
{
    ...
    #if (Driver1_NUM_SEG > 8u)
        static uint8 Driver1_dmaSegMTd[Driver1_NUM_COM];
    #endif /* (Driver1_NUM_SEG > 8u) */
    #if (Driver1_NUM_SEG > 16u)
        static uint8 Driver1_dmaSegHTd[Driver1_NUM_COM];
    #endif /* (Driver1_NUM_SEG > 16u) */

    /************************* DMA configuration for the Commons *************************/
    /* Common DMA channel */
    Driver1_dmaComChannel = Driver1_DMA_Com_DmaInitialize(1u, 1u, 
                                        HI16(Driver1_SRAM_BASE), HI16(Driver1_CONTROL_REG_BASE));

    /* Check if the channel is valid */                                                                        
    if(Driver1_dmaComChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate the common TDs */
        for(i = 0u; i < Driver1_TWO_NUM_COM; i++)
        {
            Driver1_dmaComTd[i] = CyDmaTdAllocate();
        }
        
        /* Set configuration for the Common TDs */
        for(i = 0u;  i < (Driver1_TWO_NUM_COM - 1u); i++)
        {
            /* Separate configurations for odd and even TDs */
            if((uint8)(i & 1u) == 0u)
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(Driver1_dmaComTd[i], 1u, 
                                                Driver1_dmaComTd[i+1u],
                                                TD_INC_SRC_ADR | Driver1_DMA_Com__TD_TERMOUT_EN);
            }
            else
            {
                /* Set configuration for the Common TDs */
                (void) CyDmaTdSetConfiguration(Driver1_dmaComTd[i], 1u,
                                                Driver1_dmaComTd[i+1u], TD_INC_SRC_ADR);
            }
        }
        /* Set the wraparound Common TD configuration */
        (void) CyDmaTdSetConfiguration(Driver1_dmaComTd[Driver1_TWO_NUM_COM - 1u], 
                                        1u, Driver1_dmaComTd[0u], TD_INC_SRC_ADR);
        
        /* Set the lower addresses of the common TDs */
        for(i = 0u; i < Driver1_TWO_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(Driver1_dmaComTd[i],
                                        LO16((uint32) &Driver1_comBuffer[i]),
                                        LO16((uint32) Driver1_COM_CONTROL_PTR));
        }    
    }
    ...
    Driver1_dmaSegChannel = Driver1_DMA_Seg_DmaInitialize(1u, 1u, 
                                        HI16(Driver1_SRAM_BASE), HI16(Driver1_CONTROL_REG_BASE));
    
    /* Check if the segment channel is valid */
    if(Driver1_dmaSegChannel != DMA_INVALID_CHANNEL)
    {
        /* Allocate separate Tds for segments to implement the chain */
        for(i = 0u; i < Driver1_NUM_COM; i++)
        {
            Driver1_dmaSegLTd[i] = CyDmaTdAllocate();
            #if (Driver1_NUM_SEG > 8u)
                Driver1_dmaSegMTd[i] = CyDmaTdAllocate();
            #endif /* (Driver1_NUM_SEG > 8u) */
            #if (Driver1_NUM_SEG > 16u)
                Driver1_dmaSegHTd[i] = CyDmaTdAllocate();
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
        
        /* Set the configuration for the Segment TDs */
        for(i = 0u; i < (Driver1_NUM_COM - 1u); i++)
        {
            #if (Driver1_NUM_SEG <= 8u)
                (void) CyDmaTdSetConfiguration(Driver1_dmaSegLTd[i], 1u, Driver1_dmaSegLTd[i+1u],
                                                TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(Driver1_dmaSegLTd[i], 1u, Driver1_dmaSegMTd[i],
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);

                #if (Driver1_NUM_SEG <= 16u)
                    (void) CyDmaTdSetConfiguration(Driver1_dmaSegMTd[i], 1u, Driver1_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
                #else    
                    (void) CyDmaTdSetConfiguration(Driver1_dmaSegMTd[i], 1u, Driver1_dmaSegHTd[i],
                                                    TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                    (void) CyDmaTdSetConfiguration(Driver1_dmaSegHTd[i], 1u, Driver1_dmaSegLTd[i+1u],
                                                    TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
                #endif /* Driver1_NUM_SEG <= 16u */
            #endif /* Driver1_NUM_SEG <= 8u */
        }
        ...
        #if (Driver1_NUM_SEG <= 8u)
            (void) CyDmaTdSetConfiguration(Driver1_dmaSegLTd[Driver1_NUM_COM - 1u],
                                            1u, Driver1_dmaSegLTd[0u], 
                                            TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
        #else    
            (void) CyDmaTdSetConfiguration(Driver1_dmaSegLTd[Driver1_NUM_COM - 1u],
                                            1u, Driver1_dmaSegMTd[Driver1_NUM_COM - 1u],
                                            TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
            #if (Driver1_NUM_SEG <= 16u)
                (void) CyDmaTdSetConfiguration(Driver1_dmaSegMTd[Driver1_NUM_COM - 1u],
                                                1u, Driver1_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
            #else    
                (void) CyDmaTdSetConfiguration(Driver1_dmaSegMTd[Driver1_NUM_COM - 1u],
                                                1u, Driver1_dmaSegHTd[Driver1_NUM_COM - 1u], 
                                                TD_INC_SRC_ADR | TD_AUTO_EXEC_NEXT);
                (void) CyDmaTdSetConfiguration(Driver1_dmaSegHTd[Driver1_NUM_COM - 1u],
                                                1u, Driver1_dmaSegLTd[0u], 
                                                TD_INC_SRC_ADR | Driver1_DMA_Seg__TD_TERMOUT_EN);
            #endif /* (Driver1_NUM_SEG <= 16u) */
        #endif /* (Driver1_NUM_SEG <= 8u) */
        ...
        for(i = 0u; i < Driver1_NUM_COM; i++)
        {
            (void) CyDmaTdSetAddress(Driver1_dmaSegLTd[i],
                                            LO16((uint32) &Driver1_segLBuffer[i]),
                                            LO16((uint32) Driver1_SEG_L_CONTROL_PTR));
            #if (Driver1_NUM_SEG > 8u)
                (void) CyDmaTdSetAddress(Driver1_dmaSegMTd[i],
                                            LO16((uint32) &Driver1_segMBuffer[i]),
                                            LO16((uint32) Driver1_SEG_M_CONTROL_PTR));
            #endif /* (Driver1_NUM_SEG > 8u) */
            #if (Driver1_NUM_SEG > 16u)
                (void) CyDmaTdSetAddress(Driver1_dmaSegHTd[i], 
                                            LO16((uint32) &Driver1_segHBuffer[i]),
                                            LO16((uint32) Driver1_SEG_H_CONTROL_PTR));
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
    }
    ...
    /******************** DMA configuration for Brightness control *********************/
        ...
        Driver1_dmaBCChannel = Driver1_DMA_BC_DmaInitialize(1u, 1u, 
                                        HI16(Driver1_SRAM_BASE), HI16(Driver1_CONTROL_REG_BASE));
            
        /* Check if the channels are valid */
        if(Driver1_dmaBCChannel != DMA_INVALID_CHANNEL)
        {
            Driver1_dmaBCTd = CyDmaTdAllocate();
            
            /* Set the TD configuration for the brightness DMA */
            (void) CyDmaTdSetConfiguration(Driver1_dmaBCTd, Driver1_NUM_COM,
                                        Driver1_dmaBCTd, TD_INC_SRC_ADR);
            
            /* Lower address for the brightness Td */
            (void) CyDmaTdSetAddress(Driver1_dmaBCTd, LO16((uint32) &Driver1_brightnessReg[0u]),
                                                               LO16((uint32)Driver1_COMPARE_PTR));
        }
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
}
...
/*******************************************************************************
* Function Name: Driver1_DmaEnable
********************************************************************************
*
* Summary: 
...
static void Driver1_DmaEnable(void) 
{
    ...
    (void) CyDmaChSetInitialTd(Driver1_dmaComChannel, Driver1_dmaComTd[0u]);
    (void) CyDmaChSetInitialTd(Driver1_dmaSegChannel, Driver1_dmaSegLTd[0u]);
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
        (void) CyDmaChSetInitialTd(Driver1_dmaBCChannel, Driver1_dmaBCTd);
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
    
    /* Enable all channels */
    (void) CyDmaChEnable(Driver1_dmaComChannel, 1u);
    (void) CyDmaChEnable(Driver1_dmaSegChannel, 1u);
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
        (void) CyDmaChEnable(Driver1_dmaBCChannel, 1u);
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
}


/*******************************************************************************
* Function Name: Driver1_DmaDisable
********************************************************************************
*
* Summary: 
...
static void Driver1_DmaDisable(void) 
{
    uint8 i;
    uint8 channel;
    uint8 dmaChannelList[Driver1_CHANNEL_LIMIT];
    
    dmaChannelList[0u] = Driver1_dmaComChannel;
    dmaChannelList[1u] = Driver1_dmaSegChannel;
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
        dmaChannelList[2u] = Driver1_dmaBCChannel;
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */

    for(i = 0u; i < Driver1_CHANNEL_LIMIT; i++)
    {
        channel = dmaChannelList[i];
        (void) CyDmaChDisable(channel);
        
        Driver1_Com_Driver_Write(Driver1_CLEAR);
...
{
    static uint8 Driver1_dmaInitDone = 0u;
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
        uint8 i;
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
    
    /* Start fresh by clearing the display */
    Driver1_ClearDisplayAll();
    
    /* Initialize the DMAs */
    if(0u == Driver1_dmaInitDone)
    {
        Driver1_DmaInit();
        Driver1_dmaInitDone = 1u;
    }
    ...
    /* Set the brightness to full */
        ...
        for(i = 0u; i<(Driver1_NUM_COM); i++)
        {
            Driver1_brightnessReg[i] = Driver1_INIT_BRIGHT;
        }
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
}
...
void Driver1_Enable(void) 
{
    ...
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
       /* Initialize to zero */
       Driver1_COUNTER_REG = 0x00u;
       Driver1_COMPARE_REG = 0x00u;
       
        /* Enable the PWM for brightness control */
       Driver1_CONTROL_REG = Driver1_CTRL_ENABLE;
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
    
    Driver1_enableVar = Driver1_CTRL_ENABLE;
}


/*******************************************************************************
* Function Name: Driver1_Start
********************************************************************************
*
* Summary:
...
void Driver1_Stop(void) 
{
    #if (Driver1_BRIGHTNESS_EN == Driver1_ENABLED)
        /* Disable the PWM */
        Driver1_CONTROL_REG = Driver1_CTRL_DISABLE;
    #endif /* (Driver1_BRIGHTNESS_EN == Driver1_ENABLED) */
    
    Driver1_DmaDisable();
    Driver1_ClearDisplayAll();
    
    Driver1_enableVar  = Driver1_CTRL_DISABLE;
}


/*******************************************************************************
* Function Name: Driver1_SetDisplayRAM
********************************************************************************
*
* Summary:
...
*  RAM directly by writing to Driver1_segLBuffer,
*  Driver1_segMBuffer, Driver1_segHBuffer.
*
* Parameters:
...
void Driver1_SetDisplayRAM(uint8 value, uint8 position) 
{
    ...
    if(position < Driver1_NUM_COM)
    {
        relPosition = position;
        #if(Driver1_SEG_DRIVE == Driver1_HIGH)
            Driver1_segLBuffer[relPosition] = value;
        #else
            Driver1_segLBuffer[relPosition] = ((uint8) ~value);
        #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
    }    
    else if (position < Driver1_TWO_NUM_COM)
    {
        #if (Driver1_NUM_SEG > 8u)
            relPosition = position - Driver1_NUM_COM;
            #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                Driver1_segMBuffer[relPosition] = value;
            #else
                Driver1_segMBuffer[relPosition] = ((uint8) ~value);
            #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
        #endif /* (Driver1_NUM_SEG > 8u) */
    }
    else
    {
        #if (Driver1_NUM_SEG > 16u)
            relPosition = position - Driver1_TWO_NUM_COM;
            if(relPosition < Driver1_NUM_COM)
            {
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segHBuffer[relPosition] = value;
                #else
                    Driver1_segMBuffer[relPosition] = ((uint8) ~value);
                #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
            }
...
void Driver1_SetRC(uint8 row, uint8 column) 
{
    ...
    if(column < Driver1_NUM_COM)
    {
        if(row < 8u)
        {
            rowSeg = row;
            #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                Driver1_segLBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
            #else
                Driver1_segLBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
            #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
        }
        else if (row < 16u)
        {
                ...
                rowSeg = row - 8u;
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segMBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    Driver1_segMBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
            #endif /* (Driver1_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSeg = row - 16u;
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segHBuffer[column] |= ((uint8)((uint8)0x01u << rowSeg));
                #else
                    Driver1_segHBuffer[column] &= ((uint8)~((uint8)((uint8)0x01u << rowSeg)));
                #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
    }
...
void Driver1_ClearRC(uint8 row, uint8 column) 
{
    ...
    if(column < Driver1_NUM_COM)
    {
        if(row < 8u)
        {
            ...
            #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                Driver1_segLBuffer[column] &= ((uint8)(~(uint8)rowSelect));
            #else
                Driver1_segLBuffer[column] |= rowSelect;
            #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
        }    
        else if (row < 16u)
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segMBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    Driver1_segMBuffer[column] |= rowSelect;
                #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
            #endif /* (Driver1_NUM_SEG > 8u) */
        }
        else
        {
                ...
                rowSelect = ((uint8)((uint8)0x01u << rowSeg));
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segHBuffer[column] &= ((uint8)(~(uint8)rowSelect));
                #else
                    Driver1_segHBuffer[column] |= rowSelect;
                #endif /* Driver1_SEG_DRIVE == Driver1_HIGH */
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: Driver1_ToggleRC
********************************************************************************
*
* Summary:
...
uint8 Driver1_GetRC(uint8 row, uint8 column) 
{
    ...
    if(column < Driver1_NUM_COM)
    {
        ...
        {
            rowSeg = row;
            rowSegSelect = (uint8)((uint8)Driver1_segLBuffer[column] >> rowSeg);
        }    
        else if (row < 16u)
        {
            #if (Driver1_NUM_SEG > 8u) 
                rowSeg = row - 8u;
                rowSegSelect = (uint8)((uint8)Driver1_segMBuffer[column] >> rowSeg);
            #endif /* (Driver1_NUM_SEG > 8u) */
        }
        else
        {
            #if (Driver1_NUM_SEG > 16u)
                rowSeg = row - 16u;
                rowSegSelect = (uint8)((uint8)Driver1_segHBuffer[column] >> rowSeg);
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
...
/*******************************************************************************
* Function Name: Driver1_ClearDisplay
********************************************************************************
*
* Summary:
...
{  
    ...
    {
        relPosition = position;
        Driver1_segLBuffer[relPosition] = Driver1_CLEAR;
    }    
    else if (position < Driver1_TWO_NUM_COM)
    {
        #if (Driver1_NUM_SEG > 8u)
            relPosition = position - Driver1_NUM_COM;
            Driver1_segMBuffer[relPosition] = Driver1_CLEAR;
        #endif /* (Driver1_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Driver1_NUM_COM)
            {
                Driver1_segHBuffer[relPosition] = Driver1_CLEAR;
            }
        #endif /* (Driver1_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Driver1_ClearDisplayAll
********************************************************************************
*
* Summary:
...
void Driver1_ClearDisplayAll(void) 
{
    ...
    for(position = 0u; position < Driver1_NUM_COM; position++)
    {
        Driver1_segLBuffer[position] = Driver1_CLEAR; 
        
        #if (Driver1_NUM_SEG > 8u) 
            Driver1_segMBuffer[position] = Driver1_CLEAR;
        #endif /* (Driver1_NUM_SEG > 8u) */
        
        #if (Driver1_NUM_SEG > 16u)
            Driver1_segHBuffer[position] = Driver1_CLEAR;
        #endif /* (Driver1_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Driver1_Write7SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < Driver1_NUM_COM)
    {
        choiceReg = Driver1_SEG_REG_LOW;
        relPosition = position;
    }
    else if (position < Driver1_TWO_NUM_COM)
    {
        choiceReg = Driver1_SEG_REG_MID;
        relPosition = position - Driver1_NUM_COM;
    }
    else
    {
        choiceReg = Driver1_SEG_REG_HIGH;
        relPosition = position - Driver1_TWO_NUM_COM;
    }
    
    /* Routine to extract the sign and to convert to unsigned */
    if(number >= Driver1_ZERO_NEG)
    {
        numberVal = (uint32)number;
        numberSign = Driver1_UNSIGNED;
    }
    else
    {
        numberVal = (uint32)(-number);
        numberSign = Driver1_SIGNED;
    }
    ...
    for(i = digits; i > 0u; i--)
    {
        ...
        bufArray[i-1u] = Driver1_EncodeNumber7Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    
    /* Clean up the array and place sign if needed */
    if(alignment == Driver1_ZERO_PAD)
    {
        if(numberSign == Driver1_SIGNED)
        {
            /* Place sign on the MSB */
            bufArray[0u] = Driver1_EncodeChar7Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Driver1_ZERO_7CHAR)
            {
                if(numberSign == Driver1_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Driver1_EncodeChar7Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Driver1_EncodeChar7Seg('-');
                    }
                }
    ...
    if(alignment == Driver1_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != Driver1_CLEAR)
            {
                if(relPosition < Driver1_NUM_COM)
                {
                    ...
                    switch(choiceReg)
                    {
                        case Driver1_SEG_REG_LOW :
                            Driver1_segLBuffer[relPosition] = bufArray[i];
                            break;
                        case Driver1_SEG_REG_MID :
                            #if (Driver1_NUM_SEG > 8u)
                                Driver1_segMBuffer[relPosition] = bufArray[i];
                            #endif /* (Driver1_NUM_SEG > 8u) */
                            break;
                        case Driver1_SEG_REG_HIGH :
                            #if (Driver1_NUM_SEG > 16u)
                                Driver1_segHBuffer[relPosition] = bufArray[i];
                            #endif /* (Driver1_NUM_SEG > 16u) */
                            break;
                        default :
                            Driver1_segLBuffer[relPosition] = bufArray[i];
    ...
    else
    {
        ...
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < Driver1_NUM_COM)
            {
                /* Decide which row to use */
                switch(choiceReg)
                {
                    case Driver1_SEG_REG_LOW :
                        Driver1_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        break;
                    case Driver1_SEG_REG_MID :
                        #if (Driver1_NUM_SEG > 8u)
                            Driver1_segMBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (Driver1_NUM_SEG > 8u) */
                        break;
                    case Driver1_SEG_REG_HIGH :
                        #if (Driver1_NUM_SEG > 16u)
                            Driver1_segHBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
                        #endif /* (Driver1_NUM_SEG > 16u) */
                        break;
                    default :
                        Driver1_segLBuffer[(i + relPosition) - 1u] = bufArray[i-1u];
...
/*******************************************************************************
* Function Name: Driver1_Write7SegNumberHex
********************************************************************************
*
* Summary:
...

{
    ...
    for(i = digits; i > 0u; i--)
    {
        selectVal = (uint8)((uint8)numberVal & Driver1_NIBBLE_MASK);
        bufArray[i-1u] = Driver1_EncodeNumber7Seg(selectVal);
        numberVal = (uint32)((uint32)numberVal >> Driver1_SHIFT_NIBBLE);
    }
...
/*******************************************************************************
* Function Name: Driver1_WriteString7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < Driver1_NUM_COM)
    {
        ...
        for(relPosition = position; relPosition < Driver1_NUM_COM; relPosition++)
        {
            if(relPosition < sizeLimit)
            {
                Driver1_segLBuffer[relPosition] = Driver1_EncodeChar7Seg(character[i]);
                i++;
            }
        }
    }    
    ...
    {
        #if (Driver1_NUM_SEG > 8u) 
            sizeLimit = ((uint8)(strlen(character)) + position) - Driver1_NUM_COM;
            for(relPosition = position - Driver1_NUM_COM;
                relPosition < Driver1_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    Driver1_segMBuffer[relPosition] = Driver1_EncodeChar7Seg(character[i]);
                    i++;
                }
            }
        #endif /* (Driver1_NUM_SEG > 8u) */
    ...
    {
        #if (Driver1_NUM_SEG > 16u)
            sizeLimit = ((uint8)(strlen(character)) + position) - Driver1_TWO_NUM_COM;
            for(relPosition = position - Driver1_TWO_NUM_COM;
                relPosition < Driver1_NUM_COM; 
                relPosition++)
            {
                if(relPosition < sizeLimit)
                {
                    Driver1_segHBuffer[relPosition] = Driver1_EncodeChar7Seg(character[i]);
                    i++;
                }    
            }
        #endif /* (Driver1_NUM_SEG > 16u) */
...
/*******************************************************************************
* Function Name: Driver1_PutChar7Seg
********************************************************************************
*
* Summary:
...

{
    ...
    {
        relPosition = position;
        Driver1_segLBuffer[relPosition] = Driver1_EncodeChar7Seg(character);
    }    
    else if (position < Driver1_TWO_NUM_COM)
    {
        #if (Driver1_NUM_SEG > 8u) 
            relPosition = position - Driver1_NUM_COM;
            Driver1_segMBuffer[relPosition] = Driver1_EncodeChar7Seg(character);
        #endif /* (Driver1_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Driver1_NUM_COM)
            {
                Driver1_segHBuffer[relPosition] = Driver1_EncodeChar7Seg(character);
            }
        #endif /* (Driver1_NUM_SEG > 16u) */
    }
}
...
/*******************************************************************************
* Function Name: Driver1_Write7SegDigitDec
********************************************************************************
*
* Summary:
...

{
    ...
    if (digit < 10u)
    {
        ...
        {
            relPosition = position;
            Driver1_segLBuffer[relPosition] = Driver1_EncodeNumber7Seg(digit);
        }    
        else if (position < Driver1_TWO_NUM_COM)
        {
            #if (Driver1_NUM_SEG > 8u)
                relPosition = position - Driver1_NUM_COM;
                Driver1_segMBuffer[relPosition] = Driver1_EncodeNumber7Seg(digit);
            #endif /* (Driver1_NUM_SEG > 8u) */
        } 
        else
        {
            #if (Driver1_NUM_SEG > 16u)
                relPosition = position - Driver1_TWO_NUM_COM;
                if(relPosition < Driver1_TWO_NUM_COM)
                {
                    Driver1_segHBuffer[relPosition] = Driver1_EncodeNumber7Seg(digit);
                }
            #endif /* (Driver1_NUM_SEG > 16u) */
        }
    }
...
/*******************************************************************************
* Function Name: Driver1_Write7SegDigitHex
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Driver1_Write14SegNumberDec
********************************************************************************
*
* Summary:
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = Driver1_EncodeNumber14Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == Driver1_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = Driver1_EncodeChar14Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Driver1_ZERO_14CHAR)
            {
                if(numberSign == Driver1_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Driver1_EncodeChar14Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Driver1_EncodeChar14Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
    ...
    if(alignment == Driver1_LEFT_ALIGN)
    {
        for(i = 0u; i < digits; i++)
        {
            if(bufArray[i] != Driver1_CLEAR_16)
            {
                if(relPosition < Driver1_NUM_COM)
                {
                    /* Only show displays that are not empty */
                    Driver1_segLBuffer[relPosition] = LO8(bufArray[i]);
                #if (Driver1_NUM_SEG > 8u)
                    Driver1_segMBuffer[relPosition] = HI8(bufArray[i]);
                #endif /* (Driver1_NUM_SEG > 8u) */
                }
                relPosition++;
            }
    ...
    else
    {
        for(i = digits; i > 0u; i--)
        {
            if(((i + relPosition) - 1u) < Driver1_NUM_COM)
            {
                Driver1_segLBuffer[(i + relPosition) - 1u] = LO8(bufArray[i-1u]);
            #if (Driver1_NUM_SEG > 8u)
                Driver1_segMBuffer[(i + relPosition) - 1u] = HI8(bufArray[i-1u]);
            #endif /* (Driver1_NUM_SEG > 8u) */
            }
        }  
    }
...
/*******************************************************************************
* Function Name: Driver1_Write14SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: Driver1_WriteString14Seg
********************************************************************************
*
* Summary:
...

{
    ...
    for(currPosition = position; currPosition < Driver1_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = Driver1_EncodeChar14Seg(character[i]);
            Driver1_segLBuffer[currPosition] = LO8(charDisplay);
            #if (Driver1_NUM_SEG > 8u)
            Driver1_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (Driver1_NUM_SEG > 8u) */
            i++;
        }
    }
...
/*******************************************************************************
* Function Name: Driver1_PutChar14Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display
...

{
    ...
    if(position < Driver1_NUM_COM)
    {
        charDisplay = Driver1_EncodeChar14Seg(character);
        Driver1_segLBuffer[position] = LO8(charDisplay);
    #if (Driver1_NUM_SEG > 8u)
        Driver1_segMBuffer[position] = HI8(charDisplay);
    #endif /* (Driver1_NUM_SEG > 8u) */
    }
}
...
/*******************************************************************************
* Function Name: Driver1_Write14SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < Driver1_NUM_COM))
    {
        charDisplay = Driver1_EncodeNumber14Seg(digit);
        Driver1_segLBuffer[position] = LO8(charDisplay);
    #if (Driver1_NUM_SEG > 8u)
        Driver1_segMBuffer[position] = HI8(charDisplay);
    #endif /* (Driver1_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Driver1_Write14SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: Driver1_Write16SegNumberDec
********************************************************************************
*
* Summary:
*  Displays a signed integer up to 8 characters long, starting at 'position' 
...

{
    ...
    {
        selectVal = (uint8)(numberVal % 10u);
        bufArray[i-1u] = Driver1_EncodeNumber16Seg(selectVal);
        numberVal = numberVal / 10u;
    }
    ...
    if(alignment == Driver1_ZERO_PAD)
    {
        ...
        {
            /* Place sign on the MSB */
            bufArray[0u] = Driver1_EncodeChar16Seg('-');
        }
    }
    else
    {
        for(i = 0u; i < digits; i++)
        {
            if (bufArray[i] != Driver1_ZERO_16CHAR)
            {
                if(numberSign == Driver1_SIGNED)
                {
                    if(i == 0u)
                    {
                        /* Place sign on the MSB */
                        bufArray[i] = Driver1_EncodeChar16Seg('-');
                    }
                    else
                    {
                        /* Place sign before MSB */
                        bufArray[i-1u] = Driver1_EncodeChar16Seg('-');
                    }
                }
                /* Exit when encountering non-zero number */
                break;
...
/*******************************************************************************
* Function Name: Driver1_Write16SegNumberHex
********************************************************************************
*
* Summary:
*  Displays a hexadecimal number up to 8 characters long, starting at 'position'
...
/*******************************************************************************
* Function Name: Driver1_WriteString16Seg
********************************************************************************
*
* Summary:
*  Displays a null terminated string starting at 'position' and ending at either
...

{
    ...
    for(currPosition = position; currPosition < Driver1_NUM_COM; currPosition++)
    {
        if(currPosition < sizeLimit)
        {
            charDisplay = Driver1_EncodeChar16Seg(character[i]);
            Driver1_segLBuffer[currPosition] = LO8(charDisplay);
            #if (Driver1_NUM_SEG > 8u)
                Driver1_segMBuffer[currPosition] = HI8(charDisplay);
            #endif /* (Driver1_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Driver1_PutChar16Seg
********************************************************************************
*
* Summary:
*  Displays an ascii encoded character at 'position'. This function can display 
...

{
    ...
    if(position < Driver1_NUM_COM)
    {
        charDisplay = Driver1_EncodeChar16Seg(character);
        Driver1_segLBuffer[position] = LO8(charDisplay);
        #if (Driver1_NUM_SEG > 8u)
            Driver1_segMBuffer[position] = HI8(charDisplay);
        #endif /* (Driver1_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Driver1_Write16SegDigitDec
********************************************************************************
*
* Summary:
*  Displays a single digit on the specified display. The number in 'digit' 
...

{
    ...
    if((digit < 10u) && (position < Driver1_NUM_COM))
    {
        charDisplay = Driver1_EncodeNumber16Seg(digit);
        Driver1_segLBuffer[position] = LO8(charDisplay);
        #if (Driver1_NUM_SEG > 8u)
            Driver1_segMBuffer[position] = HI8(charDisplay);
        #endif /* (Driver1_NUM_SEG > 8u) */
...
/*******************************************************************************
* Function Name: Driver1_Write16SegDigitHex
********************************************************************************
*
* Summary:
*  Displays a single digit at the specified position. The number in 'digit' 
...
/*******************************************************************************
* Function Name: Driver1_PutDecimalPoint
********************************************************************************
*
* Summary:
...

{
    ...
    if(position < Driver1_NUM_COM)
    {
        ...
        {
            #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                Driver1_segLBuffer[relPosition] &= ((uint8)(~(uint8)value));
            #else
                Driver1_segLBuffer[relPosition] |= ((uint8)(~(uint8)value));
            #endif /* (Driver1_SEG_DRIVE == Driver1_HIGH) */
        }
    ...
    else if (position < Driver1_TWO_NUM_COM)
    {
            ...
            {
                #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                    Driver1_segMBuffer[relPosition] &= ((uint8)(~(uint8)value));
                #else
                    Driver1_segMBuffer[relPosition] |= ((uint8)(~(uint8)value));
                #endif /* (Driver1_SEG_DRIVE == Driver1_HIGH) */
            }
    ...
    else
    {
            ...
            if(relPosition < Driver1_NUM_COM)
            {
                ...
                {
                    #if(Driver1_SEG_DRIVE == Driver1_HIGH)
                        Driver1_segHBuffer[relPosition] &= ((uint8)(~(uint8)value));
                    #else
                        Driver1_segHBuffer[relPosition] |= ((uint8)(~(uint8)value));
                    #endif /* (Driver1_SEG_DRIVE == Driver1_HIGH) */
                }
...
/*******************************************************************************
* Function Name: Driver1_GetDecimalPoint
********************************************************************************
*
* Summary:
...
uint8 Driver1_GetDecimalPoint(uint8 position) 
{
    ...
    {
        relPosition = position;
        decPointVal = (uint8)((uint8)(Driver1_segLBuffer[relPosition]) >> Driver1_DEC_POINT);
    } 
    else if (position < Driver1_TWO_NUM_COM)
    {
        #if (Driver1_NUM_SEG > 8u) 
            relPosition = position - Driver1_NUM_COM;
            decPointVal = (uint8)((uint8)(Driver1_segMBuffer[relPosition]) >> Driver1_DEC_POINT);
        #endif /* (Driver1_NUM_SEG > 8u) */
    }
    else
    {
            ...
            if(relPosition < Driver1_NUM_COM)
            {
                decPointVal = (uint8)((uint8)(Driver1_segHBuffer[relPosition]) >> Driver1_DEC_POINT);
            }
        #endif /* (Driver1_NUM_SEG > 16u) */
    }
    
    #if(Driver1_SEG_DRIVE == Driver1_HIGH)
        value = (uint8)(decPointVal & Driver1_DEC_POINT_MASK);
    #else
        value = (uint8)((uint8)((uint8)~decPointVal) & Driver1_DEC_POINT_MASK);
    #endif /* (Driver1_SEG_DRIVE == Driver1_HIGH) */
...
/*******************************************************************************
* Function Name: Driver1_EncodeNumber7Seg
********************************************************************************
*
* Summary:
...
uint8 Driver1_EncodeNumber7Seg(uint8 number) 
{
    ...
    #if(Driver1_SEG_DRIVE == Driver1_HIGH)
        static const uint8 CYCODE Driver1_7SegDigits[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    #endif /*  Driver1_SEG_DRIVE */

    uint8 valNum;
    uint8 valHex;
    
    valNum = (uint8)(number & Driver1_NIBBLE_MASK);
    
    valHex = Driver1_7SegDigits[valNum];
...
/*******************************************************************************
* Function Name: Driver1_EncodeChar7Seg
********************************************************************************
*
* Summary:
...
uint8 Driver1_EncodeChar7Seg(char8 input)  
{
    /* 7-segment ASCII LUT */
    #if(Driver1_SEG_DRIVE == Driver1_HIGH)
        static const uint8 CYCODE Driver1_7SegASCII[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    valLow = (uint8)((uint8)input & Driver1_NIBBLE_MASK);
    
    /* valHigh used to access the row of LUT */
    valHigh = (uint8)((uint8)input >> Driver1_SHIFT_NIBBLE);
    
    /* Check if the high nibble is within valid ASCII range */
    if ((valHigh > Driver1_SEG_ASCII_LIM_LO) && (valHigh < Driver1_SEG_ASCII_LIM_HI))
    {
        valHigh = valHigh - Driver1_SEG_ASCII_OFFSET;
        valHex = Driver1_7SegASCII[valHigh][valLow];
    }
    else
    {
        valHex = Driver1_CLEAR;
    }
...
/*******************************************************************************
* Function Name: Driver1_EncodeNumber14Seg
********************************************************************************
*
* Summary:
...
uint16 Driver1_EncodeNumber14Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE Driver1_14SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Driver1_14SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)Driver1_14SegDigitsHigh[valNum] << Driver1_SHIFT_BYTE);
    valLow = (uint16)Driver1_14SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: Driver1_EncodeChar14Seg
********************************************************************************
*
* Summary:
...
uint16 Driver1_EncodeChar14Seg(char8 input) 
{
        ...
        static const uint8 CYCODE Driver1_14SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Driver1_14SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > Driver1_SEG_ASCII_LIM_LO) && (valHigh < Driver1_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 14-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)Driver1_14SegASCIIHigh[valHigh][valLow] << Driver1_SHIFT_BYTE);
        valHexLow = (uint16)Driver1_14SegASCIILow[valHigh][valLow];
        
        valHex = valHexHigh | valHexLow;
    }
    else
    {
        valHex = Driver1_CLEAR_16;
    }
...
/*******************************************************************************
* Function Name: Driver1_EncodeNumber16Seg
********************************************************************************
*
* Summary:
...
uint16 Driver1_EncodeNumber16Seg(uint8 number) 
{
        ...
        static const uint8 CYCODE Driver1_16SegDigitsLow[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Driver1_16SegDigitsHigh[16] =
              /*--------------------------------------------------------*/
              /*  '0'    '1'    '2'    '3'    '4'    '5'    '6'    '7'  */
              /*--------------------------------------------------------*/
    ...
    valHigh = (uint16)((uint16)Driver1_16SegDigitsHigh[valNum] << Driver1_SHIFT_BYTE);
    valLow = (uint16)Driver1_16SegDigitsLow[valNum];
...
/*******************************************************************************
* Function Name: Driver1_EncodeChar16Seg
********************************************************************************
*
* Summary:
...
uint16 Driver1_EncodeChar16Seg(char8 input) 
{
        ...
        static const uint8 CYCODE Driver1_16SegASCIILow[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
        ...
        static const uint8 CYCODE Driver1_16SegASCIIHigh[6][16] =
              {
              /*--------------------------------------------------------*/
              /* 'Space' '!'    '"'    '#'    '$'    '%'    '&'    '''  */
              /*--------------------------------------------------------*/
    ...
    if ((valHigh > Driver1_SEG_ASCII_LIM_LO) && (valHigh < Driver1_SEG_ASCII_LIM_HI))
    {
        ...
        /* Get the 16-seg hex value from high and low byte LUTs */
        valHexHigh = (uint16)((uint16)Driver1_16SegASCIIHigh[valHigh][valLow] << Driver1_SHIFT_BYTE);
        valHexLow = (uint16)Driver1_16SegASCIILow[valHigh][valLow];
...
/*******************************************************************************
* Function Name: Driver1_SetBrightness
********************************************************************************
*
* Summary:
...
{   
    #if(Driver1_COM_DRIVE == Driver1_HIGH)
        static const uint8 CYCODE Driver1_tempCom[16] = {0x00u, 0x01u, 0x00u, 0x02u, 0x00u, 0x04u,
                                                                  0x00u, 0x08u, 0x00u, 0x10u, 0x00u, 0x20u,
                                                                  0x00u, 0x40u, 0x00u, 0x80u};
    #else
        static const uint8 CYCODE Driver1_tempCom[16] = {0xFFu, 0xFEu, 0xFFu, 0xFDu, 0xFFu, 0xFBu,
    ...
    if(bright == 0u)
    {
        ...
        Driver1_comBuffer[(uint8)((uint8)position << 1u) + 1u] = Driver1_CLEAR;
    }
    else
    {
        /* Restore the commons */
        Driver1_comBuffer[(uint8)((uint8)position << 1u) + 1u]
            = Driver1_tempCom[(uint8)((uint8)position << 1u) + 1u];
    }
    
    /* Update the brightness register */
    Driver1_brightnessReg[position] = bright;
}


/*******************************************************************************
* Function Name: Driver1_GetBrightness
********************************************************************************
*
* Summary:
...
{
    return(Driver1_brightnessReg[position]);
}
