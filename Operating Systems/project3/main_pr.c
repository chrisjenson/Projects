...
#define FRAMES 512      /*number of frames*/
...
page_frame_node page_table[PAGES];
	...
	while (fgets(addr, 10, addrFile) != 0){
        ...
        if(frame_Num == -1){
            ...
            for(int j = 0; j < PAGES; j++){
                if(page_table[j].page_number == page_Num){
                    frame_Num = page_table[j].frame_number;
                } 
            }
